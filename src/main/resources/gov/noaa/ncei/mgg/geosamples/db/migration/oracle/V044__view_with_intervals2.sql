...
select
    ...
    (SELECT LISTAGG(V, '|') from
        (SELECT CI.ROCK_LITH as V FROM ${schema_name}.CURATORS_INTERVAL CI
         WHERE CI.IMLGS = CS.IMLGS AND CI.PUBLISH = 'Y' and CI.ROCK_LITH is not null)
    ) as ROCK_LITHOLOGY,
    (SELECT LISTAGG(V, '|') from
        (SELECT DISTINCT CI.ROCK_MIN as V FROM ${schema_name}.CURATORS_INTERVAL CI WHERE CI.IMLGS = CS.IMLGS AND CI.PUBLISH = 'Y')
    ) AS MINERALOGY,
    (SELECT LISTAGG(V, '|') from
        (SELECT DISTINCT CR.REMARK as V FROM ${schema_name}.CURATORS_INTERVAL CI, ${schema_name}.CURATORS_REMARK CR
         WHERE CI.IMLGS = CS.IMLGS AND CI.REMARK = CR.ID and CI.PUBLISH = 'Y')
    ) AS REMARK,
    (SELECT LISTAGG(V, '|') from
        (SELECT DISTINCT CI.WEATH_META as V FROM ${schema_name}.CURATORS_INTERVAL CI
         WHERE CI.IMLGS = CS.IMLGS AND CI.PUBLISH = 'Y' AND CI.WEATH_META like 'weathering - %')
    ) AS WEATHERING,
    (SELECT LISTAGG(V, '|') from
        (SELECT DISTINCT CI.WEATH_META as V FROM ${schema_name}.CURATORS_INTERVAL CI
         WHERE CI.IMLGS = CS.IMLGS AND CI.PUBLISH = 'Y' AND CI.WEATH_META like 'metamorphism - %')
    ) AS METAMORPHISM,
    (SELECT LISTAGG(V, '|') from
        (SELECT DISTINCT CI.AGE as V FROM ${schema_name}.CURATORS_INTERVAL CI WHERE CI.IMLGS = CS.IMLGS AND CI.PUBLISH = 'Y')
    ) AS AGE,
    (SELECT LISTAGG(V, '|') from
        (SELECT DISTINCT CI.IGSN as V FROM ${schema_name}.CURATORS_INTERVAL CI WHERE CI.IMLGS = CS.IMLGS AND CI.PUBLISH = 'Y')
    ) AS INTERVAL_IGSN
